...
def create_combined_skin_nose_mask_single_face(
    label_dir, image_dir, output_dir, skin_val, nose_val
):
    """
    ...
    all_label_files = [
        f for f in os.listdir(label_dir) if f.lower().endswith("_label.png")
    ]
...
if __name__ == "__main__":
    ...
    if (
        os.path.isdir(output_binary_mask_dir)
        and len(os.listdir(output_binary_mask_dir)) > 0
    ):
