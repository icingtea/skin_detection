...
class SuperpixelFeatureExtractor:
    ...
    def extract_local_binary_patterns(self, img_path: str, slic_superpixels: cv2.ximgproc.SuperpixelSLIC) -> list[dict]:
        """
    ...
    def extract_all_features(self, img_path: Path, slic: cv2.ximgproc.SuperpixelSLIC) -> List[Dict]:
        basic_features = self.basic_superpixel_features(img_path, slic)
        lbp_features = self.extract_local_binary_patterns(img_path, slic)
