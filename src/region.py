...
class FeatureDivergence:
    @staticmethod
    def get_divergence(
        feature_vector_1: Dict, feature_vector_2: Dict
    ) -> dict[Any, float]:
        ...
        for key in keys:
            if key == "label":
                continue
            divergence_vector[key] = FeatureDivergence.euclidean_distance(
                feature_vector_1[key], feature_vector_2[key]
            )
    ...
    def euclidean_distance(val1: float | np.ndarray, val2: float | np.ndarray) -> float:
        if not (type(val1) == type(val2)):
            raise ValueError(
                f"Values are of different types. {type(val1)} != {type(val2)}"
            )
        ...
        if type(val1) == float:
            ...
            if val1.shape != val2.shape:
                raise ValueError(
                    f"Shape mismatch in arrays: {val1.shape} != {val2.shape}"
                )
