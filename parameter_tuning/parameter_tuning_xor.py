...
STUDY_NAME = "xor_tuning_study"
N_TRIALS = 100000
TRIALS_PER_LOG_FILE = 1000
LOG_INTERVAL = 50


def calculate_xor_error(pred_mask, gt_mask):
    ...
    gt_bool = gt_mask > 128
    xor_mask = np.logical_xor(pred_bool, gt_bool)
    return np.mean(xor_mask)
...
def preprocess_data():
    ...
    if not image_basenames:
        raise ValueError("unable to find gt masks")
...
def objective_xor(trial, precalculated_data, mask_handler):
    ...
    for basename, data in precalculated_data.items():
        try:
            ...
            error_rate = calculate_xor_error(predicted_mask, data["gt_mask"])
            total_xor_error += error_rate
            processed_count += 1
        except Exception:
            continue

    return 1.0 if processed_count == 0 else total_xor_error / processed_count
...
def main():
    ...
    try:
        ...
        print(f"best trial value (average XOR error): {best_trial.value:.6f}")
